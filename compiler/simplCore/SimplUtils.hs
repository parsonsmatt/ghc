  ...
  where
    ...
    abstract subst (NonRec id rhs)
      ...
      where
        ...
        -- vs_here: see Note [Which type variables to abstract over]
        vs_here = toposortTyVars $
                  filter (`elemVarSet` main_v_set) $
                  closeOverKindsList $
                  exprSomeFreeVarsList (`elemVarSet` main_v_set) rhs'

    abstract subst (Rec prs)
       = do { (poly_ids, poly_apps) <- mapAndUnzipM (mk_poly1 vs_here) ids
       ...
       where
         ...
         vs_here = toposortTyVars main_vs
